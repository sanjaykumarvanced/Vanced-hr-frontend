...
export const onlyNumberFormat = (value: string) => value.trim().replace(/[^\d]/g, '');
export const minusPlusNubmberFormat = (value: string) => value.trim().replace(/[^-^\d^.]/g, '');
export const currencyFormat = (value: string) => {
  const currency = Number(value.replace(/^(?!0\.00)\d{1,3}(,\d{3})*(\.\d\d)?$/g, ''));
  if (!currency) return value;
  return currency.toLocaleString('en', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
};

export const isNumber = (value: string) => !!Number(+value?.replace(/[^\d^.]/g, ''));

export const currencyExportFormat = (value: string) =>
  +Number(+value.replace(/[^\d^.]/g, '')).toLocaleString('en', { useGrouping: false });
...
export const uniqList = (value: any) =>
  value.reduce((map: any, item: any) => map.set(item.id, item), new Map()).values();
...
export const passwordTests = () => {
  return Yup.string()
    ...
    .test('hasNumber', 'Password must have at least 1 number', (value) => /[0-9]/.test(value || ''))
    .test('hasSymbole', 'Password must have at least 1 special character', (value) =>
      /[!"'”#$%&’()*+,-./:;<=>?@[\]^_`{|}~]/.test(value || ''),
    );
};
